...
export default function ProjectsPage() {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6">
      <div className="flex items-center gap-3 mb-5 max-w-5xl mx-auto w-full">
      ...
      <div className="max-w-5xl mx-auto w-full">
        {projects.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {projects.map((p) => (
              <div key={p.id} className="rounded-lg border border-slate-800 bg-slate-900 p-4 flex flex-col gap-2 hover:border-slate-700">
                <div className="flex items-start gap-2">
                  <div className="flex-1 min-w-0">
                    <div className="font-semibold truncate">{p.name}</div>
                    <div className="text-xs text-slate-400 flex items-center gap-1 mt-0.5"><Building2 size={12} /> {clientName(p.clientId)}</div>
                  </div>
                  <span className={`text-[10px] px-2 py-0.5 rounded-full capitalize ${statusTone[p.status] || statusTone.active}`}>{p.status}</span>
                </div>
                {p.address && <div className="text-xs text-slate-500 truncate">{p.address}</div>}
                <div className="text-[11px] text-slate-500">
                  {p.takeoff?.traces?.length || 0} traces · updated {new Date(p.updatedAt).toLocaleDateString()}
                </div>
                <div className="flex items-center gap-1.5 mt-1">
                  <button onClick={() => openProject(p.id)}
                    className="flex-1 flex items-center justify-center gap-1 text-xs py-1.5 rounded bg-brand hover:bg-brand2 font-medium">
