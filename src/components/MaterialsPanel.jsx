  ...
  return (
    <>
      <div className="px-3 pb-3">
        <div className="text-xs font-semibold text-slate-400 mb-1.5">MATERIALS — {active.layer.name}</div>
        <div className="rounded bg-slate-900 border border-slate-800 divide-y divide-slate-800">
          {active.qty === 0 && (
            <div className="p-2.5 text-[11px] text-slate-500">Trace this layer to build its material list automatically.</div>
          )}
          {active.qty > 0 && active.materials.map((m, i) => (
            <div key={i} className="p-2 text-[11px]">
              ...
              <div className="text-slate-500">
                {m.mqty.toFixed(m.u === "sheet" || m.u === "ea" ? 0 : 1)} {m.u} @ {money2(m.cost)}{" "}
                <span className="text-slate-600">({Math.round(m.waste * 100)}% waste)</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="mt-auto p-3 border-t border-slate-800 bg-slate-950 sticky bottom-0">
        ...
        <div className="text-[10px] text-slate-500 mt-0.5">NW Ohio material cost · Dovinos assemblies</div>
      </div>
    </>
  );
}
